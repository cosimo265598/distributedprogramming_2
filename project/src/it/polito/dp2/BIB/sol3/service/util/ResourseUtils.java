...
public class ResourseUtils {
	...
	public void completeBookshelves(Bookshelf b, BigInteger id) {
		UriBuilder fromBuilder = bookshelves.clone().path(id.toString());
		b.setSelf(fromBuilder.build().toString());
		/*
		 * b.setSelfItems(fromBuilder.clone().path("/items").build().toString());
		 * b.setSelfReader(fromBuilder.clone().path("/nreader").build().toString());
		 * 
		 */
	}
